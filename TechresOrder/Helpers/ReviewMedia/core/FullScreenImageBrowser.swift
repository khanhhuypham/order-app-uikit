...
public final class FullScreenImageBrowser: UIViewController {
    ...
    override public func viewDidLoad() {
        ...
        NotificationCenter.default
            .addObserver(self,
                         selector:#selector(downloadMedia(_:)),
                         name: NSNotification.Name ("downloadMedia"),   object: nil)
    }
    ...
    @objc func downloadMedia(_ notification: Notification){
        let data = notification.userInfo?["userInfo"] as? [String: Any] ?? [:]
        let type = data["type"] as? Int ?? media_type.type_error.rawValue
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0, execute: {
            if type == TYPE_IMAGE{
                JonAlert.show(message: "Hình ảnh đã lưu", andIcon: UIImage(named: "icon-check-success"), duration: 2.0)
            }else if type == TYPE_VIDEO{
                JonAlert.show(message: "Video đã lưu", andIcon: UIImage(named: "icon-check-success"), duration: 2.0)
            }else{
                JonAlert.show(message: "Xảy ra lỗi", andIcon: UIImage(named: "icon-cancel"), duration: 2.0)
            }
            self.currentImageViewer?.activityIndicator.stopAnimating()
        })
    }
