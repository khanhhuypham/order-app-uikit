...
class OrderTableViewCell: UITableViewCell {
    ...
    @IBOutlet weak var view_of_pending_quantity: UIView!
    @IBOutlet weak var lbl_pending_quantity: UILabel!
    ...
    private func mapdata(data:Order){
        ...
        if data.order_method == .EAT_IN || data.order_method == .TAKE_AWAY || data.order_method == .ONLINE_DELIVERY{
            lbl_pending_quantity.text = data.order_detail_pending_quantity.description
            view_of_pending_quantity.isHidden = data.order_detail_pending_quantity > 0 ? false : true
        }else{
            lbl_pending_quantity.text = data.order_detail_quantity.description
            view_of_pending_quantity.isHidden = data.order_detail_quantity > 0 ? false : true
        }
        ...
        btn_payment_qrcode.isHidden = (permissionUtils.GPBH_2 || permissionUtils.GPBH_3) &&
        (Constants.brand.setting?.payment_type == .pay_os || Constants.brand.setting?.payment_type == .viet_qr)
        ? false
        : true
