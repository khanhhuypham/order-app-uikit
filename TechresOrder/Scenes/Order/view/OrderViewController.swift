...
class OrderViewController: BaseViewController {
    ...
    @IBOutlet weak var width_of_textfield: NSLayoutConstraint!
    @IBOutlet weak var stackview_of_btn_status: UIStackView!
    ...
    @IBOutlet weak var btnFilterOrderMethod: UIButton!
    ...
    func firstSetup(){
        ...
        let items = ["Tại bàn", "App Food"]

        let actions: [UIAction] = items.map {
            let action = UIAction(title: $0) { action in
                self.btnFilterOrderMethod.menu = self.handleSelection(title: action.title, menu:self.btnFilterOrderMethod.menu!)
            }
            return action
        }
        
        actions[0].state = .on
        btnFilterOrderMethod.menu = UIMenu(title: "Loại đơn",children: actions)
        btnFilterOrderMethod.showsMenuAsPrimaryAction = true
        btnFilterOrderMethod.changesSelectionAsPrimaryAction = true
        btnFilterOrderMethod.setAttributedTitle(
           Utils.setAttributesForBtn(content: "Tại bàn",attributes: [.font: UIFont.systemFont(ofSize: 12, weight: .semibold)]),
           for: .normal
        )
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if permissionUtils.GPBH_1{
          ...
          stackview_of_btn_status.isHidden = true
          width_of_textfield.constant =  view_of_btn.frame.width - 130
            ...
            view_of_point.isHidden = true
            width_of_textfield.constant = 180
            stackview_of_btn_status.isHidden = false
            isBlockViewHidden(true)
    ...
    private func handleSelection(title: String, menu:UIMenu) -> UIMenu{

        var apiParameter = self.viewModel.APIParameter.value
        apiParameter.order_methods = title == "Tại bàn"
        ? [.EAT_IN, .TAKE_AWAY, .ONLINE_DELIVERY]
        : [.SHOPEE_FOOD,.GRAB_FOOD, .GO_FOOD,.BE_FOOD]

        self.viewModel.APIParameter.accept(apiParameter)
        self.clearDataAndCallApi()
        
        self.btnFilterOrderMethod.setAttributedTitle(
           Utils.setAttributesForBtn(content: title,attributes: [.font: UIFont.systemFont(ofSize: 12, weight: .semibold)]),
           for: .normal
        )
