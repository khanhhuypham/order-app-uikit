...
extension ReportBusinessAnalyticsViewModel{
    func getCategories() -> Observable<APIResponse> {
        return appServiceProvider.rx.request(.categoriesManagement(brand_id:restaurant_brand_id.value,status: status.value))
    ...
    func getReportFood() -> Observable<APIResponse> {
        return appServiceProvider.rx.request(.report_business_analytics(
            ...
            is_combo: is_combo.value,
            is_gift: is_gift.value,
            is_goods: is_goods.value,
            is_take_away_food: is_take_away_food.value))
               .filterSuccessfulStatusCodes()
               .mapJSON().asObservable()
               .showAPIErrorToast()
               .mapObject(type: APIResponse.self)
