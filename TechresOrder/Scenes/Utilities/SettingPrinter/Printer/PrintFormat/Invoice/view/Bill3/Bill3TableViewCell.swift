//
//  ReceiptPrintFormatTableViewCell.swift
//  TECHRES-ORDER
//
...
class Bill3TableViewCell: UITableViewCell {
    ...
    private func mapData(data: OrderItem){
        ...
        lbl_quantity.text = String(format:data.is_sell_by_weight == ACTIVE ?"%.2f x %@":"%.0f x %@", data.quantity,data.price.toString)
        ...
        data.order_detail_additions.enumerated().forEach{(i,value) in
            ...
            var totalPrice = value.total_price.toString
            
            if setting.is_show_vat_on_items_in_bill == ACTIVE{
                totalPrice += String(format: " (%@%%)", value.vat_percent.toString)
            }
            
            text += String(format: " + %@ x %.0f = %@\n",value.name,value.quantity,totalPrice)
        }
        ...
        if(data.is_gift == 1){
            ...
            if data.discount_amount > 0{
                ...
                lbl_amount.text = data.discount_price.toString
                
                lbl_discount_price.attributedText = Utils.setAttributesForLabel(
                    label: lbl_discount_price,
                    attributes: [(
                        str:data.total_price.toString,
                        properties:[
                            color:UIColor.white,
                ...
                lbl_amount.text = (
                    !data.order_detail_additions.isEmpty || !data.order_detail_options.isEmpty
