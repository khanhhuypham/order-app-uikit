...
extension OrderItemPrintFormatViewController {
    ...
    func drawKitchenTicket(printer:Printer,order:OrderDetail,printItems:[Food]){
        ...
        switch viewModel.printType.value{
            ...
            case .new_item://1 - món mới
                lbl_title.text = order.order_method == .EAT_IN 
                ? String(format: "TẠI BÀN - %@",order.table_name)
                : String(format: "MANG VỀ - %@",order.id_in_branch.description)
