//
//  FileStoreViewController+Extension.swift
//  TECHRES-SEEMT
//
...
extension FileStoreViewController {
    func registerCell() {
        let itemFileInStoreTableViewCell = UINib(nibName: "ItemFileInStoreTableViewCell", bundle: .main)
        tableView.register(itemFileInStoreTableViewCell, forCellReuseIdentifier: "ItemFileInStoreTableViewCell")
        tableView.separatorStyle = .none
        
        tableView.rx.modelSelected(MediaStore.self).subscribe(onNext: { [weak self] element in
            if element.media.original.url == "" &&
                element.media.original.name == "" {
                JonAlert.show(message: "Không tìm thấy file" , andIcon: UIImage(named: "icon-warning"), duration: 2.0)
            } else {
                self?.quickLook(urlString: Utils.getFullMediaLink(string: element.media.original.url), file_name: element.media.original.name)
            }
        }).disposed(by: rxbag)
   ...
   @objc func refresh(_ sender: AnyObject) {
    ...
    func bindTableViewData() {
        viewModel.dataArray.bind(to: tableView.rx.items(cellIdentifier: "ItemFileInStoreTableViewCell", cellType: ItemFileInStoreTableViewCell.self))
        {(row, item, cell) in
            cell.data = item
            if row == 0 {
...
extension FileStoreViewController: QLPreviewControllerDelegate, QLPreviewControllerDataSource {
    func numberOfPreviewItems(in controller: QLPreviewController) -> Int {
        return 1
    }

    func quickLook(urlString: String, file_name: String) {
        guard let url = URL(string: urlString) else {
            dLog("Invalid file URL")
            return
        }
        JHProgressHUD.sharedHUD.showInView(self.view)
        URLSession.shared.dataTask(with: url) { data, response, error in
            guard let data = data, error == nil else {
                JonAlert.show(message: error?.localizedDescription ?? "Không thể tải xuống file" , andIcon: UIImage(named: "icon-warning"), duration: 2.0)
                JHProgressHUD.sharedHUD.hide()
                return
            }
            do {
                let previewURL = FileManager.default.temporaryDirectory.appendingPathComponent(file_name)
