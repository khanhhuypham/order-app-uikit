...
extension OrderManagementOfFoodAppViewController {
    ...
    func getOrderHistoryOfFoodApp(){
        ...
        .mapObject(type: APIResponse.self).subscribe(onNext: {[weak self] (response) in
            ...
            if var history = Mapper<OrderHistoryOfFoodApp>().map(JSONObject: response.data){
                history.reportType = self.viewModel.history.value.reportType
                history.dateString = self.viewModel.history.value.dateString
                history.partnerId = self.viewModel.history.value.partnerId
