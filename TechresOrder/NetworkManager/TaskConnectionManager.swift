...
extension ConnectionManager {
    ...
    var task: Task {
        ...
        case .checkVersion:
            return .requestParameters(parameters: ["os_name":Utils.getOSName()],encoding: self.encoding(.get))
        ...
        case .postCreateAuthenticationCode(let expire_at,let code):
            return .requestParameters(parameters: ["expire_at": expire_at,"code":code],encoding: self.encoding(.post))
        ...
        case .forgotPassword(let username):
            ...
            return .requestParameters(parameters: ["payload": payload],encoding: self.encoding(.post))
        ...
        case .setting(let branch_id):
            return .requestParameters(parameters: ["branch_id": branch_id,"udid":Utils.getUDID(),"app_type":Utils.getAppType()],encoding: self.encoding(.get))
        ...
        case .postConfirmChannelOrder(_,let confirm_when_driver):
            return .requestParameters(
                parameters: [
                    "is_enable_confirm_when_driver":confirm_when_driver ? ACTIVE : DEACTIVE,
                    "is_enable_auto_confirm_order_channel": Constants.branch.setting.is_enable_auto_confirm_order_channel
                ],
                encoding: self.encoding(.post)
            )
                ...
                parameters: [
                    ...
                    "foods": foods.map{item in
                        ...
                        var parameters: [String: Any] = [
                            ...
                            "addition_foods": item.addition_foods.toJSON(),
                            "buy_one_get_one_foods": item.buy_one_get_one_foods.toJSON(),
        ...
        case .workingSessions(let branch_id, let employee_id):
            return .requestParameters(parameters: ["branch_id":branch_id,"employee_id": employee_id],encoding: self.encoding(.get))
        ...
        case .assignWorkingSession(let branch_id, let order_session_id):
            return .requestParameters(parameters: ["branch_id": branch_id,"order_session_id":order_session_id],encoding: self.encoding(.post))
        ...
        case .updatePrinter(let branch_id, let printer):
            return .requestParameters(
                parameters: [
                    ...
                    "is_have_printer":printer.is_have_printer,
                    "is_print_each_food":printer.is_print_each_food,
        ...
        case .report_revenue_by_time(let restaurant_brand_id, let branch_id, let report_type, let date_string, let from_date, let to_date):
            ...
            return .requestParameters(parameters: parameter,encoding: self.encoding(.get))
        ...
        case .getFoodsOfBuffetTicket(let brand_id, let buffet_ticket_id):
            ...
            case .getCouponList(let brandId,let branchId):
                return .requestParameters(parameters: ["restaurant_brand_id": brandId,"branch_id": branchId],encoding: self.encoding(.get))
            
            case .postApplyCoupon(let branchId,_, let couponId):
                return .requestParameters(parameters: ["branch_id": branchId,"coupon_id": couponId],encoding: self.encoding(.post))
        ...
        case .getClosedSessionHistory(let restaurant_id, let branch_id, let area_id, let from, let to, let key_search, let limit, let page):
            return .requestParameters(parameters: [
                "restaurant_id": restaurant_id,
                "branch_id": branch_id,
                "area_id": area_id,
...
//                encoding: self.encoding(.get))
        ...
        case .postBatchCancelOrderOfFoodApp(let branch_id,let ids):
            return .requestParameters(parameters: [
                "branch_id":branch_id,
                "ids":ids
            ],encoding: self.encoding(.post))
        ...
        case .getOrderHistoryOfFoodApp(let brand_id, let branch_id, let food_channel_id, let report_type, let date_string, let page, let limit):
            return .requestParameters(
                parameters: [
                    "restaurant_brand_id":brand_id,
                    "branch_id":branch_id,
...
//                    "hour_to_take_report":hour_to_take_report
        ...
        case .getRevenueSummaryReportOfFoodApp(let brand_id,let branch_id, let date_string, let report_type):
            return .requestParameters(
                parameters: [
                    "restaurant_brand_id":brand_id,
                    "branch_id":branch_id,
        ...
        case .postCancelItemOfFoodApp(let orderId,let itemId):
            return .requestParameters(
                parameters: [
                    "id":itemId,
        ...
        //=======================================================================================================================================================================================================
            ...
            case .getPartnerInvoiceConnectionDetail(let partner_electronic_invoice_type):
                return .requestParameters(parameters: [
                    "partner_electronic_invoice_type":partner_electronic_invoice_type.rawValue
                ],encoding: self.encoding(.get))
            
            case .postChangeStatusPartnerInvoiceConnection(_, let is_confirm):
                return .requestParameters(parameters: ["is_confirm":is_confirm],encoding: self.encoding(.post))
            ...
            case .postUpdatePartnerInvoiceConnection(let invoice):
                ...
                var parameter:[String:Any] = [
                    ...
                    "invoice_denominator": invoice.invoice_denominator,
                    "apply_discount":invoice.apply_discount,
                    "endpoint": invoice.endpoint,
                    "is_auto_export_third_party": invoice.is_auto_export_third_party,
                    "apply_order_types":  invoice.apply_order_types
                ]
                if invoice.partner_electronic_invoice_type == .vnpt{
                    parameter.updateValue(invoice.username_access_service, forKey: "username_access_service")
                    parameter.updateValue(invoice.password_access_service, forKey: "password_access_service")
                }
            ...
            case .postCreatePartnerInvoiceConnection(let branch_id,let invoice):
            
                var parameter:[String:Any] = [
                    "branch_id": branch_id,
                    "partner_invoice_id":invoice.partner_invoice_id,
                    "partner_identify_name":invoice.partner_invoice_name,
                    "tax_code":invoice.tax_code,
                    "username":invoice.username,
                    "password":invoice.password,
                    "invoice_series":invoice.invoice_series,
                    ...
                    "apply_discount":invoice.apply_discount,
                    "apply_order_types":invoice.apply_order_types,
                    "partner_electronic_invoice_type":invoice.partner_electronic_invoice_type.rawValue
                ]
                if invoice.partner_electronic_invoice_type == .vnpt{
                    parameter.updateValue(invoice.username_access_service, forKey: "username_access_service")
                    parameter.updateValue(invoice.password_access_service, forKey: "password_access_service")
            ...
            case .getEInvoiceList(let apply_order_typ,let key_search, let branch_id, let limit, let page, let from, let to,let cct_duyet,let invoice_status):
            
                var parameter:[String:Any] = [
                    "apply_order_typ": apply_order_typ,
                    "key_search":key_search,
                    "branch_id":branch_id,
                    "limit":limit,
                    "page":page,
            ...
            case .postAssignBranchForEInvoicePartner(let invoice):
            ...
            case .postUnassignBranchForEInvoicePartner(let invoice):
            
                var parameter:[String:Any] = [:]
            
                if let branch_assigns = invoice.branch_assigns.first{
                    parameter.updateValue([branch_assigns.id], forKey: "branch_ids")
                    parameter.updateValue(invoice.partner_electronic_invoice_type.rawValue, forKey: "partner_electronic_invoice_type")
                }
